...
const makeRequest = url => asyncRequest(url, null)
...
const parseData = (html, host) => {
  ...
  const articles = [...links]
    ...
    .filter(item =>
      $(item).attr('class').indexOf('article') > -1 ||
      $(item).attr('class').indexOf('item-title') > -1)
    .map(item => ({
      title: $(item).text().replace(/(\r\n|\n|\r)/gm, '').trim(),
      date: $(item).siblings().find('[class*="date"]').text()
        .replace(/[\n\t\r]/g, '')
        .trim() || // harpers bazaar
              $(item).parent().find('[class*="date"]').children()
                .text()
                .replace(/[\n\t\r]/g, '')
                .trim() || // conde nast
              $(item).parent().siblings().find('[class*="date"]')
                .text()
                .replace(/[\n\t\r]/g, '')
                .trim(), // cosmo
...
const scrapeArticles = siteList => Promise.all(flatten(siteList.map((site) => {
  if (!site.subs || site.subs.length === 0) {
    return makeRequest(site.baseUrl).then(res => parseData(res.body, res.socket._host));
  }
  return [
    ...site.subs.map(sub => makeRequest(site.baseUrl + sub)
      .then(res => parseData(res.body, site.baseUrl))),
